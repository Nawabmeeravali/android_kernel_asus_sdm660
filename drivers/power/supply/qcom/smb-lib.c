...
/* Huaqin add for ZQL1650-1287 factory version remove before BC1.2 500mA before adapter id 1000mA by fangaijun at 2018/5/8 end */
		...
		if (HVDCP_FLAG == 0) {
			...
			queue_delayed_work(system_power_efficient_wq,&smbchg_dev->asus_adapter_adc_work, msecs_to_jiffies(ADC_WAIT_TIME_HVDCP0));
		} else {
			printk("%s: NOT factory_build, HVDCP_FLAG = 2or3, ADC_WAIT_TIME = 0.1s\n", __func__);
			queue_delayed_work(system_power_efficient_wq,&smbchg_dev->asus_adapter_adc_work, msecs_to_jiffies(ADC_WAIT_TIME_HVDCP23));
		}
#endif
...
/* Huaqin add for ZQL1650-68 Realize jeita function by fangaijun at 2018/02/03 start */
		...
		if (!asus_flow_processing) {
			...
			//queue_delayed_work(system_power_efficient_wq,&smbchg_dev->asus_set_flow_flag_work, msecs_to_jiffies(2000));         //TYPE-C
			asus_insertion_initial_settings(smbchg_dev);
			asus_smblib_stay_awake(smbchg_dev);
			#if 0
			if (g_Charger_mode)
				queue_delayed_work(system_power_efficient_wq,&smbchg_dev->asus_chg_flow_work, msecs_to_jiffies(12500));
			else
